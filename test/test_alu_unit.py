...
async def test_alu_random(dut):
    ...
    for op_name, op_code, op_func in [
        ...
        ("MUL", ALU_MUL, lambda a, b: ((a & 0xFF) * (b & 0xFF)) & 0xFF),
        ("DIV", ALU_DIV, lambda a, b: (a // b) if b != 0 else 0),
    ]:
        mismatches = 0
        for _ in range(num_tests):
            if op_code == ALU_DIV:
                rt = random.choice([1, 2, 3, 4, 8, 9, 10, 11, 12, 16, 32, 64])
                if rt in [3, 9, 10, 11, 12]:
                    rs = random.randint(0, 255)
                else:
                    rs = random.randint(0, 0xFFFF)
            elif op_code == ALU_MUL:
                rs = random.randint(0, 255)
