...
from helpers.q115 import (
    float_to_q115, q115_to_float, q115_add,
...
def q115_activation_ref(x: int, bias: int, func: int) -> int:
    """Apply bias and activation function in SF16 (sign-magnitude)."""
    return q115_activation(x, func, bias)
...
async def test_activation_passthrough(dut):
    ...
    for x_f, bias_f, desc in test_cases:
        ...
        expected = q115_activation_ref(x_q, bias_q, ACT_NONE)
...
async def test_activation_relu(dut):
    ...
    for x_f, bias_f, desc in test_cases:
        ...
        expected = q115_activation_ref(x_q, bias_q, ACT_RELU)
...
async def test_activation_leaky_relu(dut):
    ...
    for x_f, bias_f, desc in test_cases:
        ...
        expected = q115_activation_ref(x_q, bias_q, ACT_LEAKY_RELU)
...
async def test_activation_clipped_relu(dut):
    ...
    for x_f, bias_f, desc in test_cases:
        ...
        expected = q115_activation_ref(x_q, bias_q, ACT_CLIPPED_RELU)
...
async def test_activation_all_functions(dut):
    ...
    for x_f, bias_f, desc in test_inputs:
        ...
        for func in [ACT_NONE, ACT_RELU, ACT_LEAKY_RELU, ACT_CLIPPED_RELU]:
            ...
            expected = q115_activation_ref(x_q, bias_q, func)
