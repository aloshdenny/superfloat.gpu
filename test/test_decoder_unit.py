...
async def decode_instruction(dut, instruction: int) -> dict:
    ...
    result = {
        ...
        'systolic_enable': int(dut.decoded_systolic_enable.value),
        'systolic_op': int(dut.decoded_systolic_op.value),
        'systolic_idx': int(dut.decoded_systolic_idx.value),
...
@cocotb.test()
async def test_decoder_sys(dut):
    """Test SYS systolic-array instruction decoding."""
    logger = await setup_decoder_test(dut, "decoder_sys")

    passed = True
    test_cases = [
        (0b00, "CLEAR", 0, {'reg_write_enable': 0, 'systolic_idx': 0}),
        (0b01, "LOAD", 0, {'reg_write_enable': 0, 'systolic_idx': 0}),
        (0b10, "COMPUTE", 0, {'reg_write_enable': 0, 'systolic_idx': 0}),
        (0b11, "READ_0", 0, {'reg_write_enable': 1, 'reg_input_mux': REG_MUX_SYS, 'systolic_idx': 0}),
        (0b11, "READ_1", 1, {'reg_write_enable': 1, 'reg_input_mux': REG_MUX_SYS, 'systolic_idx': 1}),
    ]

    for op, name, idx, extra_expected in test_cases:
        instr = (OP_SYS << 12) | (5 << 8) | (op << 6) | idx
        logger.log_message(f"  SYS.{name} R5: 0x{instr:04X}")
        ...
        if not check_signals(decoded, expected, logger):
            ...
            logger.log_message(f"    PASS: systolic_op={decoded['systolic_op']}, systolic_idx={decoded['systolic_idx']}")

    logger.log_message(f"\nOverall: {'PASS' if passed else 'FAIL'}")
    logger.close()

    assert passed, "Decoder SYS test failed"
...
async def test_decoder_all_opcodes(dut):
    ...
    opcodes = [
        ...
        (OP_SYS, "SYS.READ", {'reg_write_enable': 1, 'systolic_enable': 1, 'systolic_op': 0b11, 'reg_input_mux': REG_MUX_SYS}),
        (OP_RET, "RET", {'ret': 1}),
    ]
