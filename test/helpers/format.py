...
def decode_instruction(instr: int) -> str:
    ...
    elif opcode == 0b1011:  # ACT
        return f"ACT R{rd}, R{rs}, R{rt}"
    elif opcode == 0b1100:  # SYS
        op = (instr >> 6) & 0x3
        names = ["CLEAR", "LOAD", "COMPUTE", "READ"]
        return f"SYS.{names[op]} R{rd}"
...
def format_thread_state(
    ...
    lines.append(f"LSU State: {LSU_STATES.get(lsu_state, 'UNKNOWN')} (en={mem_read_en})")
