...
module mem_coalesce #(
    ...
    // Input requests from LSUs (flattened for Yosys compatibility)
    ...
    genvar u;
    generate
        for (u = 0; u < NUM_REQUESTS; u = u + 1) begin : unflatten
    ...
    // Flatten outputs (parameterized)
    integer fl_idx;
    always @(*) begin
        for (fl_idx = 0; fl_idx < NUM_REQUESTS; fl_idx = fl_idx + 1) begin
