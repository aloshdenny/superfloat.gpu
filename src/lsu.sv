...
// LOAD-STORE UNIT (SF16 Fixed-Point)
// > Handles asynchronous memory load and store operations and waits for response
// > Each thread in each core has its own LSU
// > LDR, STR instructions are executed here
// > Supports 16-bit SF16 data transfers
module lsu #(
    parameter ADDR_BITS = 8,   // Address width (8-bit for 256 memory locations)
    parameter DATA_BITS = 16  // Data width (16-bit for SF16 fixed-point)
) (
    input wire clk,
    input wire reset,
    ...
    assign mem_read_address = ADDR_BITS'(rs);
    assign mem_write_address = ADDR_BITS'(rs);
                ...
                case (lsu_state)
                    ...
                    WAITING: begin
                        if (mem_read_ready == 1) begin
                            mem_read_valid <= 0;
                            // Load 16-bit SF16 value from memory
