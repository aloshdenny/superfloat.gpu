.PHONY: test compile compile_tb clean test_all_units compile_librelane clean_librelane help view_layout

export LIBPYTHON_LOC=$(shell cocotb-config --libpython)
export PYGPI_PYTHON_BIN=$(shell cocotb-config --python-bin)

# =============================================================================
# Configuration
# =============================================================================

# KLayout executable path (macOS)
KLAYOUT := /Applications/KLayout/klayout.app/Contents/MacOS/klayout
KLAYOUT_LYP := scripts/sky130.lyp

# =============================================================================
# Original GPU testbench targets
# =============================================================================

test_%:
	make compile_tb
	COCOTB_TEST_MODULES=test.test_$* vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp

compile:
	@mkdir -p build
	sv2v src/*.sv -w build/gpu.v
	echo '`timescale 1ns/1ns' | cat - build/gpu.v > build/temp.v
	mv build/temp.v build/gpu.v

compile_tb:
	@mkdir -p build
	sv2v src/*.sv test/tb_gpu.sv -w build/all.v
	echo '`timescale 1ns/1ns' | cat - build/all.v > build/temp.v
	mv build/temp.v build/all.v
	# RAM32.v is plain Verilog (sta-blackbox model); append after sv2v
	cat src/RAM32.v >> build/all.v
	iverilog -o build/sim.vvp -s tb_gpu -g2012 build/all.v

# =============================================================================
# Unit Test Targets for Individual Modules
# =============================================================================

# FMA Unit
compile_fma:
	@mkdir -p build build/waves
	sv2v src/fma.sv test/tb_fma.sv -w build/fma.v
	echo '`timescale 1ns/1ns' | cat - build/fma.v > build/temp.v
	mv build/temp.v build/fma.v
	iverilog -o build/fma.vvp -s tb_fma -g2012 build/fma.v

test_fma_unit: compile_fma
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_fma_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/fma.vvp

# ALU Unit
compile_alu:
	@mkdir -p build build/waves
	sv2v src/alu.sv test/tb_alu.sv -w build/alu.v
	echo '`timescale 1ns/1ns' | cat - build/alu.v > build/temp.v
	mv build/temp.v build/alu.v
	iverilog -o build/alu.vvp -s tb_alu -g2012 build/alu.v

test_alu_unit: compile_alu
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_alu_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/alu.vvp

# Activation Unit
compile_activation:
	@mkdir -p build build/waves
	sv2v src/activation.sv test/tb_activation.sv -w build/activation.v
	echo '`timescale 1ns/1ns' | cat - build/activation.v > build/temp.v
	mv build/temp.v build/activation.v
	iverilog -o build/activation.vvp -s tb_activation -g2012 build/activation.v

test_activation_unit: compile_activation
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_activation_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/activation.vvp

# Systolic PE Unit
compile_systolic_pe:
	@mkdir -p build build/waves
	sv2v src/systolic_pe.sv test/tb_systolic_pe.sv -w build/systolic_pe.v
	echo '`timescale 1ns/1ns' | cat - build/systolic_pe.v > build/temp.v
	mv build/temp.v build/systolic_pe.v
	iverilog -o build/systolic_pe.vvp -s tb_systolic_pe -g2012 build/systolic_pe.v

test_systolic_pe_unit: compile_systolic_pe
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_systolic_pe_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/systolic_pe.vvp

# Systolic Array Unit
compile_array:
	@mkdir -p build build/waves
	sv2v src/systolic_pe.sv src/systolic_array.sv test/tb_systolic_array.sv -w build/array.v
	echo '`timescale 1ns/1ns' | cat - build/array.v > build/temp.v
	mv build/temp.v build/array.v
	iverilog -o build/array.vvp -s tb_systolic_array -g2012 build/array.v

test_array_unit: compile_array
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_systolic_array_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/array.vvp

# Cache Unit
compile_cache:
	@mkdir -p build build/waves
	sv2v src/cache.sv test/tb_cache.sv -w build/cache.v
	echo '`timescale 1ns/1ns' | cat - build/cache.v > build/temp.v
	mv build/temp.v build/cache.v
	iverilog -o build/cache.vvp -s tb_cache -g2012 build/cache.v

test_cache_unit: compile_cache
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_cache_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/cache.vvp

# Decoder Unit
compile_decoder:
	@mkdir -p build build/waves
	sv2v src/decoder.sv test/tb_decoder.sv -w build/decoder.v
	echo '`timescale 1ns/1ns' | cat - build/decoder.v > build/temp.v
	mv build/temp.v build/decoder.v
	iverilog -o build/decoder.vvp -s tb_decoder -g2012 build/decoder.v

test_decoder_unit: compile_decoder
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_decoder_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/decoder.vvp

# LSU Unit
compile_lsu:
	@mkdir -p build build/waves
	sv2v src/lsu.sv test/tb_lsu.sv -w build/lsu.v
	echo '`timescale 1ns/1ns' | cat - build/lsu.v > build/temp.v
	mv build/temp.v build/lsu.v
	iverilog -o build/lsu.vvp -s tb_lsu -g2012 build/lsu.v

test_lsu_unit: compile_lsu
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_lsu_unit vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/lsu.vvp

# =============================================================================
# Run All Unit Tests
# =============================================================================

test_all_units: test_fma_unit test_alu_unit test_activation_unit test_fma_unit test_array_unit test_cache_unit test_decoder_unit test_lsu_unit
	@echo "All unit tests completed"

# =============================================================================
# New Integration Test Targets (Q1.15 stress suite)
# =============================================================================

# Q1.15 Vector and Arithmetic Operations
test_vector_ops: compile_tb
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_vector_ops vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp

# Systolic Array Stress (standalone array DUT)
test_systolic_stress: compile_array
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_systolic_stress vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/array.vvp

# Very Large Matmul (9×9 to 32×32, multi-dispatch)
test_matmul_vvlarge: compile_tb
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_matmul_vvlarge vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp

# Extended NN Forward Pass (on-GPU activations, 3-layer MLP, attention, residual)
test_nn_forward: compile_tb
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_nn_forward vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp

# Convenience groupings
test_stress: test_systolic_stress
	@echo "Systolic stress tests completed"

test_nn: test_nn_forward
	@echo "Neural network forward pass tests completed"

# =============================================================================
# Run All Integration Tests
# =============================================================================

test_integration: test_matmul test_matadd test_matmul_large test_matmul_vvlarge test_inference test_nn_forward test_vector_ops
	@echo "All integration tests completed"

# =============================================================================
# Physical Layout Viewing (KLayout)
# =============================================================================

view_layout:
	@echo "Opening GDS in KLayout with Sky130 layer colors..."
	$(KLAYOUT) -l $(KLAYOUT_LYP) gds/atreides.gds &

# =============================================================================
# Cleanup
# =============================================================================

clean:
	rm -rf build/*.v build/*.vvp test/logs/*.log test/results/*.log build/waves/*.vcd

# =============================================================================
# Waveform Visualization
# =============================================================================

# Generic waveform viewer — usage: make show_fma, make show_alu, etc.
show_%: build/waves/%.vcd
	@# Prefer loading the saved GTKWave layout if it exists (auto-adds signals).
	@( \
		if [ -f test/gtkwave/$*.gtkw ]; then \
			gtkwave $< test/gtkwave/$*.gtkw; \
		else \
			gtkwave $<; \
		fi \
	) &

# =============================================================================
# Help
# =============================================================================

help:
	@echo "Atreides GPU v3.0 — Build System"
	@echo ""
	@echo "Architecture: 2 cores, 16 threads/block, 32× 128x128 systolic arrays per core (983,040 PEs)"
	@echo ""
	@echo "Unit Tests:"
	@echo "  make test_fma_unit            - Test FMA (Q1.15 fused multiply-add)"
	@echo "  make test_alu_unit            - Test ALU (integer operations)"
	@echo "  make test_activation_unit     - Test Activation unit"
	@echo "  make test_fma_unit    - Test FMA"
	@echo "  make test_array_unit - Test 4x4 Systolic Array"
	@echo "  make test_cache_unit          - Test Instruction Cache"
	@echo "  make test_decoder_unit        - Test Instruction Decoder"
	@echo "  make test_lsu_unit            - Test Load-Store Unit"
	@echo "  make test_all_units           - Run all unit tests"
	@echo ""
	@echo "Integration Tests:"
	@echo "  make test_matmul              - Matrix multiplication test (2×2)"
	@echo "  make test_matadd              - Matrix addition test (2×2)"
	@echo "  make test_matmul_large        - Large matmul tests (4×4, 8×8, boundary, random)"
	@echo "  make test_matmul_vvlarge      - Very large matmul (9×9..32×32, multi-dispatch)"
	@echo "  make test_inference           - NN inference workloads (MLP, conv1d, latency)"
	@echo "  make test_vector_ops          - Q1.15 vector & arithmetic ops (add, scale, dot)"
	@echo "  make test_systolic_stress     - Systolic array hardware stress (11 tests)"
	@echo "  make test_nn_forward          - NN forward pass: 3-layer MLP, GPU-ReLU, attention"
	@echo "  make test_stress              - Alias for test_systolic_stress"
	@echo "  make test_nn                  - Alias for test_nn_forward"
	@echo "  make test_integration         - Run all integration tests"
	@echo ""
	@echo "Compilation:"
	@echo "  make compile                  - Compile GPU to Verilog (sv2v)"
	@echo "  make compile_tb               - Compile with testbench"
	@echo "  make compile_librelane         - Prepare RTL + stubs for Librelane 2"
	@echo ""
	@echo "Waveforms:"
	@echo "  make show_<module>            - View waveforms (show_fma, show_alu, etc.)"
	@echo ""
	@echo "Physical Layout:"
	@echo "  make view_layout              - Open GDS in KLayout GUI"
	@echo ""
	@echo "Cleanup:"
	@echo "  make clean                    - Remove generated files"
	@echo "  make clean_librelane           - Remove Librelane runs"

# =============================================================================
# Librelane — RTL Preparation
# =============================================================================

# Prepare Verilog files for Librelane
compile_librelane:
	@echo "Preparing hierarchical Librelane build with stubs..."

	@mkdir -p \
		librelane/systolic_pe/src \
		librelane/systolic_array/src \
		librelane/core/src \
		librelane/gpu/src \
		librelane/stubs

	# ============================================================
	# 1. Generate real RTL (used only for macro builds)
	# ============================================================

	sv2v src/systolic_pe.sv \
		-w librelane/systolic_pe/src/systolic_pe.v

	sv2v src/systolic_pe.sv src/systolic_array.sv \
		-w librelane/systolic_array/src/systolic_array.v

	sv2v src/*.sv \
		-w librelane/core/src/core.v

	sv2v src/*.sv \
		-w librelane/gpu/src/gpu.v

	# ============================================================
	# 2. Generate blackbox stubs
	# ============================================================

	@echo "Generating blackbox stubs..."

	./scripts/make_stub.sh \
		librelane/systolic_pe/src/systolic_pe.v \
		librelane/stubs/systolic_pe_stub.v

	./scripts/make_stub.sh \
		librelane/systolic_array/src/systolic_array.v \
		librelane/stubs/systolic_array_stub.v

	./scripts/make_stub.sh \
		librelane/core/src/core.v \
		librelane/stubs/core_stub.v

	@echo "Librelane RTL + stubs prepared."

# Clean Librelane runs
clean_librelane:
	rm -rf librelane/*/runs
	rm -rf librelane/*/src/*.v

test_model_resnet: compile_tb
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_model_resnet vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp

test_model_transformer: compile_tb
	@mkdir -p test/results build/waves
	COCOTB_TEST_MODULES=test.test_model_transformer vvp -M $$(cocotb-config --lib-dir) -m $$(cocotb-config --lib-name vpi icarus) build/sim.vvp
